...
const listenForPayments = async () => {
  ...
  const randomSignatureCreated = await Signature.findOne({ signature: "randomSignature" });
          ...
          try {
            ...
            if (transaction !== undefined) {
              ...
              if (allSignatures.length > 0) {
                const signatureExists = allSignatures.find((eachSignature) => eachSignature.signature == signature);

                // Store transaction and credit user if it hasn't been saved in db
                if (!signatureExists) {
                  const signatureForThisTransaction = new Signature({ signature });
                  await signatureForThisTransaction.save();
                  ...
                  // console.log(`${sender}, ${amount} sol, ${creditAmount} usd`);
                    ...
                    await User.findOneAndUpdate({ walletAddress: senderAddress }, { balance: newBalance });

                    // Create new credit notification
                    const creditNotification = new Notification({
                      message: `Deposit confirmed ✅ 
Amount: *${amount} sol*
*$${parseFloat(creditAmount.toFixed(2))}* ([Using bitfinex rate](https://trading.bitfinex.com/t/SOL:USD?type=exchange))
